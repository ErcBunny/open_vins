...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < path_algorithms.size(); i++) {
    ...
    if (path_algo_datasets.find(path_gt.stem().string()) == path_algo_datasets.end()) {
      PRINT_INFO(RED "[COMP]: %s dataset does not have any runs for %s!!!!!\n" RESET, path_algorithms.at(i).filename().c_str(),
                  path_gt.stem().c_str());
      continue;
    }
    ...
    if (file_paths.empty()) {
      PRINT_INFO(RED "\tERROR: No runs found for %s, is the folder structure right??\n" RESET, path_algorithms.at(i).filename().c_str());
      continue;
    }
